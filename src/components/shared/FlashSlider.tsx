...
export function FlashSlider({ value, onValueChange }: Props) {
  ...
  return (
    ...
    >
      {/* Dim base icon */}
      <Ionicons name="flash" size={H} color={theme.borderDefault} />

      {/* Rising fill — absolutely positioned clip window growing from bottom */}
      <View style={[styles.fillClip, { height: fillH }]}>
        {/* Full-size icon pinned to bottom of this full-H inner view */}
        <View style={styles.fillIconWrap}>
          <Ionicons name="flash" size={H} color={color} />
        </View>
      </View>

      {/* Outline on top */}
      <View style={styles.overlay}>
        <Ionicons name="flash-outline" size={H} color={theme.textDisabled} style={{ opacity: 0.4 }} />
      </View>
    </View>
  );
}
